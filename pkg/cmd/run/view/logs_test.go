...
func TestZipLogFetcher(t *testing.T) {
	...
	fileWriter, err := zw.Create("foo.txt")
	assert.NoError(t, err)

	_, err = fileWriter.Write([]byte("blah blah"))
	assert.NoError(t, err)
	...
	assert.Equal(t, "foo.txt", zr.File[0].Name)
...
func TestGetLogMap(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			httpStubs: func(reg *httpmock.Registry) {
				reg.Register(
					httpmock.REST("GET", "repos/OWNER/REPO/actions/jobs/123/logs"),
					httpmock.StatusStringResponse(http.StatusOK, "job log"),
				)
			},
			zipReader: createZipReader(t, map[string]string{
				// There's no job log file, so it should be fetched from the API.
		...
		},
		{
			...
			wantJobLog: "job log",
			wantStepLogs: []any{
				errors.New("no log fetcher for step 1 of job 123"),
				"step two log",
			},
		},
		{
		...
		},
		{
			...
			wantStepLogs: []any{
				errors.New("no log fetcher for step 1 of job 123"),
				errors.New("no log fetcher for step 2 of job 123"),
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantJobLog != "" {
				gotJobLog, err := io.ReadAll(jobLogReader)
				require.NoError(t, err)
				require.Equal(t, tt.wantJobLog, string(gotJobLog))
			}
			...
			for i, wantStepLog := range tt.wantStepLogs {
				...
				case error:
					...
					require.Nil(t, stepLogReader)
				case string:
					require.NoError(t, err)
					require.NotNil(t, stepLogReader)

					defer func() {
						require.NoError(t, stepLogReader.Close())
					}()

					gotStepLog, err := io.ReadAll(stepLogReader)
					require.NoError(t, err)
					require.Equal(t, wantStepLog, string(gotStepLog))
				}
			}
		})
	}
