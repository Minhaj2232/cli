...
func (l *logs) forStep(jobID int64, stepNumber int) (io.ReadCloser, error) {
	...
	if !ok {
		return nil, fmt.Errorf("no log fetcher for step %d of job %d", stepNumber, jobID)
	}
	return f.GetLog()
}
...
func getLogs(rlz *zip.Reader, httpClient *http.Client, repo ghrepo.Interface, jobs []shared.Job) logs {
	...
	for _, job := range jobs {
		...
		if zf := matchFileInZIPArchive(rlz, jobLogFilenameRegexp(job)); zf != nil {
			...
			// If we didn't find a job log in the zip, we will try to fetch it from the API.
			jobLogFetcher = &apiLogFetcher{
